...
// -- STATE --
const API = '[localhost](http://localhost:8080/api)';
const rooms = {};
...
function getRoom(feature, code)        { return rooms[feature] && rooms[feature][code]; }
function setRoom(feature, code, data)  { if (!rooms[feature]) rooms[feature] = {}; rooms[feature][code] = data; }
...
function renderVoteBtns(room, adminRole) {
  ...
  row.innerHTML = '';
  if (room.closed || adminRole) return;
  room.items.forEach((item, i) => {
...
function showFinalResult(room) {
  ...
  resultChartInstance = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets:[{ data, backgroundColor: colors.slice(0,data.length), borderWidth:2, borderColor:'#cfc5bb' }]
    },
    options: {
      plugins:{ legend:{ labels:{ font:{family:"'Lora', serif",size:13}, color:'#1a1a1a' } } },
...
function setupFessPage(room, adminRole) {
  ...
  if (adminRole) {
    ...
    document.getElementById('fess-list').innerHTML =
      '<p style="font-family:Lora,serif;color:#888;text-align:center;padding:2rem;font-style:italic">Pesanmu akan terkirim anonim ke admin. Kamu tidak bisa melihat pesan orang lain.</p>';
  }
}
...
function votePost(postId, val) {
  const prev = myVotedItems[postId];
  const room = getRoom(currentFeature, currentRoomCode);
  if (!room) return;
  const post = room.posts.find(p => p.id===postId);
  if (!post) return;
  if (prev === 'up'   && val===1)   return;
  if (prev === 'down' && val===-1)  return;
...
async function fetchRoomFromBackend(feature, code) {
  try {
    ...
    setRoom(feature, code, {
      ...
      items: data.items||[], messages: data.messages||[], posts: data.posts||[],
      showEarlyResult: data.showEarlyResult||false, votesCast: {}
    });
    return true;
  ...
  try {
    ...
    if (feature === 'anonvote' && data.items) {
      ...
      data.posts.forEach(p => {
        ...
        else { exists.score = p.score; const el=document.getElementById('score-'+p.id); if(el) el.textContent=p.score; }
      });
      resortForumList(room);
    }
